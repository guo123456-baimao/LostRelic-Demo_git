...
第三章讲的是「这个项目实际怎么做出来的」，这一章是从里面抽出来的**可复用部分** ——
换一个项目、换一个引擎仍然成立的东西。

### 一条核心经验

> **让 AI 能自己验证，比让 AI 一次写对更重要。**

AI 生成一次的成本几乎是零，**验证一次的成本才是瓶颈**。所以精力不该花在
「怎么把 prompt 写好让它一次写对」，而该花在「怎么让它写错了能立刻知道」——
前者靠运气，后者是架构和工具链问题，可以一次性投资、长期收益。

这个项目里的三笔投资，共同点是**都不改善生成质量、只改善反馈速度**：
...
切片小 → 每次只有一个变量 → 出问题时归因是确定的。反过来一次改 5 处，
5 个静默失败叠在一起就没法反推了。每个切片走同样五步：

1. **定边界。** 这次只改什么，以及**改完怎么算通过** —— 必须是一个可观测的判据，
   不能是「感觉对了」。比如 Animator 幂等那次的判据就是「手动执行菜单会回
   `already correct, nothing written`」。
2. **先核实机制，再写代码。** 让 AI 把相关调用链读一遍、把约束说出来，然后才动手。
   做击退前先确认了三条：不能做成 FSM 状态、位移只能走 `agent:Move`、起手要清 `nav_dest`。
   **这三条决定了实现形态，是在写第一行之前定下来的，不是写完再改的。**

   第一条尤其值得看它是怎么定的 —— 它不是猜的，是算出来的：最小警戒半径 6.5 m
   远大于玩家攻击距离 2.5 m，**玩家不可能从警戒范围外命中敌人**，而 alert 块每帧都会
   重新断言 `state = "chase"`，所以任何 `state = "knockback"` 下一帧必被覆盖。
   结论是击退只能做成**叠加在状态机之上的计时器字段**。这一步省掉的是「写完、发现被覆盖、
   再回头重构状态机」那一整轮。
3. **只改一处。**
4. **自己验证。** 进 Play、读日志；涉及量级的问题就注入探针取数。
5. **把结论写进提交信息**（见 `git log`）：机制约束 + 实测数字。下一轮迭代不必重新发现
   同一件事 —— 「击退不能做成新的 FSM 状态」这条写清楚一次，就不会有人再试第二遍。

### 注意事项

**一、「能编译」离「行为正确」很远，而且中间不报错。**
第三章那 5 个案例全部是零报错。所以「跑起来没报错」不能算验证通过，
AI 说「已完成」更不能 —— 那句话只说明它写完了，不说明它对。

**二、它最不可靠的地方是引擎语义，不是语法。** 写业务逻辑很稳，一碰这三类就要盯着：
...
**三、`or` / `??` 这类兜底写法要单独警惕。**
它会静默吃掉配置里刻意填的 `0` / `false` / 空串，而 AI 很爱写，因为看起来「安全」（案例 5）。
凡是「用户可能刻意填 0」的字段，都得用 `~= nil` 判断而不是 `or`。

**四、测量不能跨工具调用。**
一次 MCP 往返会把帧撑长到 `dt = 0.333 s`，两次调用读到的根本不是同一时刻的状态。
推广一下就是：**带外观测会扰动被观测对象**，要测准就得在被测系统内部测。

**五、`git diff` 不一定能直接读。**
fileID 重分配那类改动会让 git 把互不相干的 YAML 块对齐到一起，逐行看像是逻辑被改过（案例 4）。
这种必须按语义比对 —— 这一条对 AI 和对人一样成立。
