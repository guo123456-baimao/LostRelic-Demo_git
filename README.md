...
AI 一次能生成很多代码，但上面 5 个案例说明「能编译」离「行为正确」很远。切片小 → 每次只有一个变量 →
出问题时归因是确定的；反过来一次改 5 处，5 个静默失败叠在一起就没法反推了。

配套的两条习惯：

- **让 AI 能自己验证，比让 AI 一次写对更重要。** 前者是架构和工具链问题（Lua + JSON + MCP），
  可以一次性投资；后者靠运气。
- **把机制约束和实测数字写进提交信息**（见 `git log`），下一轮迭代不必重新发现同一件事。
  比如「击退不能做成新的 FSM 状态」这个约束 —— 警戒半径 6.5 m 远大于玩家攻击距离 2.5 m，
  玩家不可能从警戒范围外命中，所以任何新状态下一帧就会被 alert 块的 `state = "chase"` 覆盖 ——
  写清楚一次，就不会有人再试第二遍。
